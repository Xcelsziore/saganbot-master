 ...
 *      dbPath : the path to access the database (will default to "data/saganbot.db")
 *
...
SaganBot.prototype._replyWithDescription = function (originalMessage) {
    ...
    self.db.get('SELECT DESCRIPTION FROM GLOSSARY WHERE TERM =\'queryMessage\'', function (err, record) {
        if (err) {
...
SaganBot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(this.channels[0].name, 'Hello there!' +
        '\n vRA can be a bummer for a first-time user. Just type a vRA-specific term you wish to know about or `' + this.name + '` to invoke my glossary!',
        {as_user: true});
};
